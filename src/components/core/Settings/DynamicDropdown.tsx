...
function DynamicDropdown<T extends OptionItem>({
    ...
    const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        const item = options.find(option => String(option[valueField]) === value) || null;
        setSelectedItem(item);
    };

    return (
        <div className={`w-full max-w-md p-4 flex flex-col gap-5 rounded `}>
            <label className="block text-gray-700 text-[0.875rem] font-medium mb-2" htmlFor={`dropdown-${title.toLowerCase().replace(/\s+/g, '-')}`}>{title}</label>
                    ...
                    >
                        {options.map((option) => (
                            <option key={option.id} value={String(option[valueField])}>
                                {option[displayField]}{secondaryField ? ` (${option[secondaryField]})` : ''}
                            </option>
                        ))}
                    </select>
